...
import { geomToPolys } from './map-layers.js';
  ...
  if (e.key === 'Escape') {
    if (st._bridgeMode) exitBridgeMode();
      ...
      confirmIfDirty(function(){
        import('./map-view.js').then(function(m){ m.openMapView(st._activeJobFolder || null); });
      });
    }
  }
});
...
map.on('contextmenu', function(e) {
  if (st._mvMode) {
    import('./map-view.js').then(function(m){ if (m.getMvSelected().size > 0) m.mvClearSel(); });
    return;
  }
