...
Fallback use — standalone stdio (for headless/automation without the web UI):
    ...
    # Claude Desktop config: {"command": "jobgen", "args": ["mcp"], "env": {"MML_API_KEY": "..."}}
"""
...
# Standalone-mode config (only used when NOT mounted inside jobgen serve)
_cfg = None
...
def _is_integrated() -> bool:
    """True when this module is running inside the jobgen serve FastAPI process."""
...
def _pipeline_guard():
    ...
    Standalone mode: uses the cross-process file lock so a running jobgen serve
...
def jobs_list_resource() -> str:
    ...
    from jobgen.job_store import scan_jobs
...
def job_detail_resource(path: str) -> str:
    ...
    from jobgen.job_store import resolve_job_dir
...
def get_job(path: str) -> str:
    ...
    """
    from jobgen.job_store import resolve_job_dir, read_job_card
    folder, name, job_dir = resolve_job_dir(_output_dir(), path)
    if not job_dir.exists():
        return json.dumps({"error": f"Job not found: {path}"})
