...
def test_fetch_forecast_uses_fresh_cache(tmp_path):
    ...
    cache_path.write_text(
        json.dumps(
            {
                "v": wx._CACHE_VERSION,
                "days": [asdict(d) for d in res.days],
                "utc_offset_s": res.utc_offset_s,
                "hourly_cloud": res.hourly_cloud,
            }
        ),
        encoding="utf-8",
    )
...
def test_load_cache_tolerates_stale_schema(tmp_path):
    p = tmp_path / "w.json"
    p.write_text(
        json.dumps({"days": [{"old_field": 1}], "utc_offset_s": 0}), encoding="utf-8"
    )
...
def test_build_day_slots_clear_window_flag():
    ...
    s = wx.build_day_slots(
        res, ops, daytime_start_h=6, daytime_end_h=18, clear_sky_max_cloud_pct=30
    )
    ...
    s2 = wx.build_day_slots(
        res2, ops, daytime_start_h=6, daytime_end_h=18, clear_sky_max_cloud_pct=30
    )
...
def test_build_day_slots_night_pass_not_clear():
    ...
    res = WeatherResult(
        days=[DayWeather("2026-06-16", 0, "clear", "Clear", 15.0, 3.0, 0.0, 5.0)],
        utc_offset_s=10800,
        hourly_cloud={"2026-06-16T00": 5.0},
    )
    slots = wx.build_day_slots(
        res, ops, daytime_start_h=6, daytime_end_h=18, clear_sky_max_cloud_pct=30
    )
...
def test_build_day_slots_per_tile_cloud():
    ...
    rep = WeatherResult(
        days=[DayWeather("2026-06-15", 0, "clear", "Clear", 15.0, 3.0, 0.0, 10.0)],
        utc_offset_s=10800,
        hourly_cloud={"2026-06-15T11": 10.0},
    )
    other = WeatherResult(
        days=[], utc_offset_s=10800, hourly_cloud={"2026-06-15T11": 90.0}
    )
    slots = wx.build_day_slots(
        rep,
        ops,
