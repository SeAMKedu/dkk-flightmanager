...
class TestFootprintSpacings:
    def test_positive_and_within_footprint(self):
        ...
        full_w = (
            80
            * _DRONE.image_width_px
            * _DRONE.pixel_pitch_um
            * 1e-6
            / (_DRONE.focal_length_mm * 1e-3)
        )
        assert strip < full_w
...
class TestPlanRouteSimple:
    def test_returns_planned_route_with_flat_altitude(self):
        pr = plan_route(
            _POLY, drone=_DRONE, height_m=80, overlap_front_pct=80, overlap_side_pct=70
        )
